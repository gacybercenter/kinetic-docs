...
CAUTION: Destroying the entire environment isn't always a necessity during rebuild, and make sure you are in the correct salt master before proceeding with these steps
...
=== Zeroize Physical OpenStack Nodes:

[#destroy_compute]
==== Compute Nodes

NOTE: This will power off all of the physical compute and container nodes and remove their public keys
...
IMPORTANT: If rebuilding Ceph, this will power off all of the physical storage nodes as well as remove their public keys.
...
IMPORTANT: If rebuilding the entire environment, this will power off all of the physical controller nodes as well as remove their public keys.
...
salt pxe* cmd.run 'rm /var/www/html/assignments/*'
...
NOTE: Order of operations matter to meet dependency requirements that are built into the automation
...
NOTE: Due to current development for a *TNSR* the your NAT configurations may need to be manually pointed to the correct internal address for the haproxy service.
...
IMPORTANT: If using network-ovn as the network backend, the ovsdb node needs to be created, otherwise disregard.
...
[,bash]
----
# Dependincies Copleted in Prior Phases
