...
NOTE: Only destroy Ceph pools if not rebuilding Ceph, refer to <<destroy_ceph, Destroy Ceph>>.
...
NOTE: If rebuilding Ceph, perform the commands below. Currently, you are required to run the commands to turn off all of the storage and cephmon nodes as well as remove the keys.
